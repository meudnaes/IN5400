...
class logreglayer(nn.Module):
    def __init__(self, dims):
        ...
        self.w = torch.nn.Parameter(data=torch.normal(mean=0., std=0.1*torch.ones((dims,1))),
                                    requires_grad=True)
    ...
    def forward(self, x):
        ...
        z = torch.mm(x, self.w.double()) + self.bias.double()
...
def run():
    ...
    dtr = torch.utils.data.TensorDataset(torch.tensor(xtr), torch.tensor(ytr).type(torch.DoubleTensor))

    # TensorDataset from tensors from xv, yv - our validation features and labels
    dv = torch.utils.data.TensorDataset(torch.tensor(xv), torch.tensor(yv).type(torch.DoubleTensor))
    ...
    #model and loss
    #TODO
    model = logreglayer(dim)
    #TODO
    criterion = torch.nn.BCELoss()

    optimizer = torch.optim.SGD(model.parameters(), lr = learningrate, momentum=0.0,
                                weight_decay=0)
    ...
    dte = torch.utils.data.TensorDataset(torch.tensor(xtr), torch.tensor(ytr).type(torch.DoubleTensor))
    loaderte = torch.utils.data.DataLoader(dte, batch_size=valbatch_size, shuffle=False)

    test_accuracy = evaluate(model = model, dataloader = loaderte,
